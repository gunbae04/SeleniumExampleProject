...
<jsp:include page="../common/header.jsp" flush="true"/>   
            ...
            <div >
					<div class="form-group form-material floating"
						...
						<input type="text" class="form-control" name="title"
							id="title" placeholder="제목을 입력하세요."/> <label class="floating-label">제목sss</label>
							
							
					</div>
					<div class="row">
                        <div class="form-group col-md-4">
                          <label class="form-control-label" for="department">담당부서</label>
                          <select class="form-control" id="departmentBox">
                            <option value="">선택하세요.</option>
                            <c:if test='${fn:length(deptList) > 0 }'>
                                <c:forEach items="${deptList}" var="dept">
                                    <option value="${dept.codeId}"><c:out value="${dept.codeName}"/></option>
                                </c:forEach>
                            </c:if>
		                   </select>
                        </div>
                        <div class="form-group col-md-4">
                          <label class="form-control-label" for="issueType">이슈타입</label>
                          <select class="form-control" id="issueType">
                          <option value="">선택하세요.</option>
                             <c:if test='${fn:length(issueList) > 0 }'>
                                  <c:forEach items="${issueList}" var="issue">
                                      <option value="${issue.codeId}"><c:out value="${issue.codeName}"/></option>
                                  </c:forEach>
                              </c:if>
		                   </select>
                        </div>
                        <div class="form-group col-md-4">
                          <label class="form-control-label" for="importance">중요도</label>
   ...
   <div class="note-editor note-frame panel panel-default">
      ...
      <div class="modal" aria-hidden="false" tabindex="-1" role="dialog" aria-label="Help">
        ...
        <div align="right">    
			<button type="button" class="btn btn-primary ladda-button add_board_btn"
